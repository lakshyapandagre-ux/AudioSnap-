...
export default function UploadTab({ onFileSelect, onFileClear, disabled }: Props) {
  ...
  const handleFile = useCallback(
    (f: File) => { setFile(f); onFileSelect(f); },
    [onFileSelect],
  );
  ...
  return (
    <div
      className={`converter__dropzone ${dragging ? 'dragover' : ''} ${file ? 'has-file' : ''}`}
      ...
      {file ? (
        <>
          <p className="converter__dropzone-text">
            <strong>{trunc(file.name, 30)}</strong>
          </p>
          <div className="file-info">
            <span>{formatSize(file.size)}</span>
            <span>·</span>
            <button className="converter__file-remove" onClick={clearFile} title="Remove file">✕</button>
          </div>
        </>
      ) : (
        <>
